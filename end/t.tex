...
wymagającym doświa\-dczenia oraz licznych eksperymentów.
...
oso\-bników, selekcję, krzyżowanie i mutację.
...
\begin{itemize}
  ...
  \item Część binarna:
    \begin{itemize}
      \setlength{\itemsep}{0pt}
      \setlength{\parskip}{0pt}
      \item liczba warstw ukrytych: 7 bitów (1-128)
      \item ziarno generatora liczby neuronów: 10 bitów (0-1023)
      \item ziarno generatora typów aktywacji: 10 bitów (0-1023)
      \item optymalizator: 1 bit (0->SGD, 1->Adam)
      \item wskaźnik uczenia się (learning rate): 13 bitów ($2^{-13}$ - $1.0$)
      \item ? wskaźnik uczenia się (learning rate): 12 bitów ($2^{-13}$ - $0.5$)
      \item liczba epok: 10 bitów (1-1024)
    \end{itemize}
  \item Część list - generowane po części binarnej;
    ...
    \begin{itemize}
      ...
      \item lista ilości neuronów w warstwach ukrytych -\\
        \hspace*{1em}zawiera liczby z zakresu 2-1 048 577\\
        \hspace*{1em}każdy element składa się z 20 bitów.
      \item lista typów aktywacji w warstwach ukrytych -\\
        \hspace*{1em}zawiera liczby: 0->relu, 1->tanh, 2->sigmoid\\
...
gdzie losowo są wybrane punkty podziału w obu listach.
Natomiast o ile pierwszy punkt podziału jest wybierany z pełnego zakresu
...
i przekształcone na przystosowanie.
...
która dała najlepszą dokładność na danych walidacyjnych.z 
Jeżeli dokładność ta będzie posiadała kilka epok mających
taką samą wartość, to zostanie wybrana ta z najwcześniejszej epoki. % może lepsze z późniejszej?
