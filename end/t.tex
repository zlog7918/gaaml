...
Natomiast nawet posiadając doświadczenie nie jest to pewne,
że dany model będzie odpowiednio działał ani,
że jest najlepszym z możliwych.
Konsekwencją tego jest potrzeba licznych eksperymentów z różnymi modelami,
...
\noindent\\
Potrzeba ograniczenia ręcznego zmieniania parametrów i testowania,
kieruje do automa\-tyzacji tego procesu.
...
z pomocą algorytmu genetycznemu pod względem architektury sieci
i jej parametrów uczenia na podstawie danych treningowych
...
w spójny sposób, architektury sieci neuronowej oraz para\-metrów uczenia.

\noindent\\
Kolejnym jest zawarcie w kodzie nauczania sieci neuronowych
wraz z zatrzymaniem tego procesu na jej najbardziej optymalnym poziomien nauczania.

\noindent\\
Ostatnim krokiem jest raportowanie wszystkich prób do pliku wraz
z ich nauczonymi wagami.
% Raportowanie wszystkich prób do pliku wraz
% z ich nauczonymi wagami jest następnym krokiem.

% \noindent\\
% Ostatnim z kamieni milowych będzie 
...
\noindent\\
Optymalizacja powinna się odbywać dla konkretnych danych dostarczonych
przez uży\-tkownika, za pomocą algorytmu genetycznego.
...
Program nie operuje na wszystkich typach sieci neuronowych,
tworzy tylko sieci MLP, więc po użyciu stworzonego
w niniejszej pracy programu, można dalej zoptymalizować sieć.
Do jeszcze lepszego działania mogą się przyczynić:
połączenia przechodzące na inne warstwy niż tylko następna w kolejności
i/lub mniejsza ilość połączeń między neuronami (czyli gdy przestaną być w pełni połączone).

\noindent\\
Uczenie sieci neuronowej jest operacją zużywającą
dużą ilość mocy obliczeniowej.
Dodatkowo ponie\-waż proces uczenia sieci zależy
od pierwotnie wylosowanych wag, to dla uzyskania większej pewności
co do uzyskanego wyniku należy proces uczenia wykonać kilkakrotnie.
Powoduje to, że działanie programu jest znacząco uzależnione
od mocy obliczeniowej urządzenia wykonującego operacje
i proces optymalizacji poprzez algorytm genetyczny
...
o jakości porównywalnej lub lepszej niż rozwiązania dobierane manualnie
przy ograniczeniu nakładu pracy użytkownika.

\subsection*{Skrócone omówienia}
Tu jeszcze nic nie ma,
ponieważ będą na bieżąco dodawane krótkie opisy do danego
rozdziału po skończeniu jego skończeniu.
...
%           $\left[2, 1 048 577\right]$,\\
%         \hspace*{1em}każdy element składa się z 20 bitów.
%       \item lista typów aktywacji w warstwach ukrytych -\\
%         \hspace*{1em}liczba elementów - o 2 większa
%           (uwzględnia warstwę wejściową i wyjściową):
%           $\left[3, 130\right]$,\\
%         \hspace*{1em}zawiera liczby z zakresu:
%           $\left[0, 3\right]$
%           (%
...
%   na zbiorze $\mathbb{N} \cap \left[p, k\right]$,\\
% $r_1\left[i\right]$ i $r_2\left[i\right]$ -
%   rodzice 1 i 2 dla
%   $i \in \mathbb{N} \cap \left[1, n\right]$,\\
% $c_1\left[i\right]$ i $c_2\left[i\right]$ -
%   dzieci 1 i 2 dla
%   $i \in \mathbb{N} \cap \left[1, n\right]$:
...
%   c_1\left[i\right]=\begin{cases}
%     p_1[i]
%       &, i \le p_r\\
%     p_2[i]
%       &, i>p_r\\
%   \end{cases}\\
%   c_2\left[i\right]=\begin{cases}
%     p_2[i]
%       &, i \le p_r\\
%     p_1[i]
%       &, i>p_r\\
...
% $n_{c_1}$ i $n_{c_2}$ - liczba elementów dla dziecka 1 i 2,\\
% $c_1\left[i_{c_1}\right]$ i $c_2\left[i_{c_2}\right]$ -
%   dzieci 1 i 2 odpowiednio dla
%   $i_{c_1} \in \mathbb{N} \cap \left[1, e_s n_{c_1}\right]$
%   i $i_{c_2} \in \mathbb{N} \cap \left[1, e_s n_{c_2}\right]$:
...
%     0 &, l_b=0
%   \end{cases}\\
%   l_{p_{c_1}}=\left\lfloor
%     \frac{p_{r_1}}{e_s}
%   \right\rfloor + l_{p_b}\\
%   l_{k_{c_2}}=n_{r_1}-\left\lfloor
%     \frac{p_{r_1}}{e_s}
%   \right\rfloor
% $
...
%     n_{r_2}-l_{p_b}
%       &, l_{p_{c_1}} \ge l_{\min}\\
%     n_{r_2}-\left(l_{\min}-l_{p_{c_1}}+l_{p_b}\right)
%       &, l_{p_{c_1}}<l_{\min}\\
%   \end{cases}\\
%   p_{r_2}=e_s R\left(p_r, k_r\right) + l_b
% $
...
%   \end{cases}\\
%   c_1\left[i_{c_1}\right]=\begin{cases}
%     p_1[i_{c_1}]
%       &, i_{c_1} \le p_{r_1}\\
%     p_2[i_{c_1}-p_{r_1}+p_{r_2}]=
%       p_2[i_{c_1}-{\Delta p_r}]
%       &, i_{c_1}>p_{r_1}\\
%   \end{cases}\\
%   c_2\left[i_{c_2}\right]=\begin{cases}
%     p_2[i_{c_2}]
%       &, i_{c_2} \le p_{r_2}\\
%     p_1[i_{c_2}-p_{r_2}+p_{r_1}]=
%       p_1[i_{c_2}+{\Delta p_r}]
%       &, i_{c_2}>p_{r_2}\\
%   \end{cases}
% $
