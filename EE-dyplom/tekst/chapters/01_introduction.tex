...
ozpatrywać jako problem optymalizacyjny
...
\begin{nospitemize}
    \item Czy zastosowanie algorytmu genetycznego
      pozwala uzyskać modele o~jakości porównywalnej
      lub wyższej niż modele projektowane manualnie?
...
Przeprowadzenie eksperymentów obejmuje
sprawdzenie czy automatycznie dobrane sieci
są porównywalnej lub lepszej jakości
jak wybrane manualnie oraz porównanie
określonych parametrów dla algorytmu genetycznego,
by wybrać domyślne parametry
dla algorytmu genetycznego.
Parametry obejmują liczbę generacji
i~osobników w~populacji
oraz współczynnik krzyżowania i~mutacji.

\section*{Ograniczenia}
W~pracy rozważane są wyłącznie
sieci MLP w~pełni połączone.
Nie uwzględnia się nie w~pełni połączonych MLP,
...
z~szczególnym uwzględnieniem MLP.
...
Będą dalsze opisy na bieżąco dodawane
dla danych rozdziałów po ich skończeniu.
