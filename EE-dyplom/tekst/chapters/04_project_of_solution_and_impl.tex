...
biblioteka powinna udostępniać dwie funkcje.
...
Funkcja ta tworzy model, uczy go
i~oblicza dopasowanie jego odpowiedzi
do danych walidacyjnych, a~jeżeli
niedostępne to treningowych, oraz testowych.
Pierwsza z~odpowiedzi będzie zwana
dalej dopasowaniem walidacyjnym.
Dopasowanie, oraz uzyskane po korekcji przystosowanie,
z~danych testowych nie będzie wykorzystywane
do żadnego procesu ewolucji,
pozwala jedynie na ujrzenie pełnego obrazu,
jak się ta wartość zmieniała w~czasie.
Następnie przed zwróceniem obydwu dopasowań
zapisuje wagi nauczonego modelu do pliku
by użytkownik mógł je potem w~łatwy sposób wgrać,
a~do drugiego pliku wpisywane są następujące metadane:
liczba epok, wielkość wsadu,
backend, liczba warstw ukrytych,
historia dopasowań podczas kolejnych epok nauki,
dopasowanie walidacyjne oraz dopasowanie testowe.

Obiekt pokoleń tworzy na początku
swojego działania obiekt populacji.
...
Podczas tworzenia obiektu populacji od razu
generowana jest populacja początkowa
i~obliczana jest dla niej wartość przystosowania.
...
\begin{nospitemize}
  \item Część sekwencyjna:
    \begin{nospitemize}
      ...
      % \item wskaźnik uczenia się (learning rate):
        % 11 bitów $2^{-13}\left(
        %   \mathbb{Z} \cap \left[1, 2^{11}\right]
        % \right)$
      \item liczba epok:
...
\begin{figure}
  \centering
  \scalebox{0.95}{\input{img/04_img/gen_parts.tikz.tex}}
  {%
...
\begin{figure}
  \centering
  \input{img/04_img/bin_gen.tikz.tex}
  \caption{%
...
by zachować spójność długości list oraz by zapobiec występowaniu ``półelementów''.
...
Decyzja o~mutacji będzie podejmowana
...
Obliczona dokładność walidacyjna zostaje przekształcona
na przystosowanie za pomocą funkcji
...
oraz $d$ jest błądem dopasowania
...
Więc w~miejscu $n$ w~\cref{eq:correction_f} jest w~bibliotece osobnik z~którego można
...
Po zakończeniu uczenia zostanie obliczona
dokładność na danych treningowych i~testowych,
...
Wartość poprawionego kodu $v\left(b'\right)$
z~\cref{math:corrected_bin} jest na pewno
mniejsza lub równa $v\left(m\right)$,
dodatkowo jeżeli $
...
\begin{nospitemize}
  ...
  \item optymalizator
    ...
    \Rightarrow$ SGD,
  % TODO: maybe this version
  %   2^(-13)*[1 - 2^12] → 2^(-13)[1 - 2^11]
  % \item wskaźnik uczenia się (learning rate):
    % -- 11 bitów $
    %   {2^{-13}} {\cdot} {\left[1, 2^{11}\right]}
    % $
  \item wskaźnik uczenia się (learning rate)
    -- 12 bitów $
      {2^{-13}} {\cdot} {\left[1, 2^{12}\right]}
    $
...
\begin{figure}
  \begingroup
    ...
    \centering
    \input{img/04_img/correction_of_bin.tikz.tex}
    \caption{%
      ...
      fioletowym~(\showcolor{blue!75!yellow}).
      ...
      kolorem czerwonym~(\showcolor{red}).
      Skorygowany kod został oznaczony
      kolorem zielonym~(\showcolor{green!30})
      z~szczególnym uwzględnieniem zmienionych
      bitów ciemniejszym kolorem~(\showcolor{green})%
    }
    \label{fig:bin_corr}
  \endgroup
\end{figure}  
...
Proces ten opisany jest w~\cref{subsec:crossover_nni}.
...
Proces ten został opisany w~\cref{subsec:crossover_nni}.
...
\begin{figure}
  \centering \input{img/04_img/ind_fenotype.tikz.tex}
  \caption{%
